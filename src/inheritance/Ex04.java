...
//		System.out.println(h3.major);
		...
		// 다형성에 의해 서로 다른 타입으로 참조할 수 있지만, 타입에 다라 참조가능한 필드 범위가 달라진다

		h1.show(); // show() 메서드는 Human 타입에 정의되어 있어서 호출이 가능하다
		h2.show(); // 각 객체의 메서드는 서브클래스에 의해 오버라이딩 (덮어씌우기) 되어있다
		h3.show(); // 호출이 가능하지만, 호출한 후 실행되는 내용은 덮어씌운 내요으로 실행된다
		h4.show(); // 강제 형변환이 없어도 서브 클래스의 내용대로 실행된다 // 오버라이딩 되지 않은 메서드는 강제 형변환이 필요하다
