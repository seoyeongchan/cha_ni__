...
class Score{	// 객체를 만들기 위한 설계도의 역할을 수행하는 클래스
	int kor, eng, mat;	// 객체의 세부 [속성]값을 저장할 변수(멤버필드)
	
	void show() {		// 객체의 [기능]을 표현하는 함수(멤버메서드)
 ...
 *  - 생성자 : 객체를 생성할 때 호출되는 특수한 함수, 오버로딩이 가능하다. 무조건 클래스의 이름과 동일한 함수
...
public class Ex02 {
	public static void main(String[] args) {
		// 1) 세 과목의 점수를 배열로 처리함
		int[] arr = {100, 99, 87};
		System.out.println("국어 점수 : " + arr[0]);	// 0은 국어점수
		System.out.println("영어 점수 : " + arr[1]);	// 1은 영어점수
		System.out.println("수학 점수 : " + arr[2]);	// 2는 수학점수이다 (라고 기억해야함)
		System.out.println();
		
		// 2) 세 과목의 점수를 클래스에 의한 객체로 처리함
		Score ob = new Score();		// 객체를 생성하기 위해서는 무조건 new
		ob.kor = 100;
		ob.eng = 99;
		ob.mat = 87;
